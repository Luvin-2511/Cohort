...
const App = () => {
  ...
  return (
    <div className='h-screen w-full bg-[#0F0A1F] p-4 text-white'>
      <form onSubmit={(e) => {
        submitter(e)
      ...
      <div className='conta h-1/2 lg:w-[45rem] flex flex-col m-auto mt-8  bg-[#201c2f00] rounded-3xl p-3 '>
        ...
        {alltask.map((elem, idx) => {
          return <div className='relative overflow-hidden insider border-[0.2px] cursor-pointer flex justify-between hover:scale-[1.02] transition-all items-center px-4 py-2 rounded-xl mt-2 border-gray-400'>
            <div className='relative z-10 flex items-center  gap-4'>
              ...
              <label className='h-[2rem] w-[2rem] cursor-pointer rounded-full bg-gray-600 flex items-center justify-center' htmlFor={`tracker-${idx}`}>
            ...
            <div className='relative z-10 flex gap-2'>
              <button onClick={() => {
                editor(idx)
              }} className='p-2 cursor-pointer hover:scale-[1.04]'>Edit</button>
              <button onClick={() => {
                deleter(idx)
