...
const MainCanvas = () => {
      ...
      setRects((prev) =>
        prev.map((rect) =>
          rect.id === dragRef.current.shapeId
            ? {
                ...rect,
                x: newx + dragRef.current.startLeft,
                y: newy + dragRef.current.startTop,
              }
            : rect,
        ),
      );
  ...
  const dragShaper = (e) => {
    ...
    const shapeData =
      rects.find((r) => r.id === shape.id) ||
      circs.find((c) => c.id === shape.id);
    setShapeProperty({
      x: shapeData.x,
      y: shapeData.y,
      rot: 0,
      ...
      {circs.map((circ, idx) => {
        return (
          <div
            ...
            className={`shape h-25 w-25 hover:border-2 rounded-full ${selectedShape === circ.id ? "border-2 border-blue-500" : ""} hover:border-blue-500 rounded translate-[-50%] bg-[#d9d9d9] absolute`}
          >
                ...
                {handles.map((handle) => {
                  return (
                    <div
                      ...
                      key={handle.id}
                      id={handle.id}
                      className={`handle h-2 w-2 ${handle.id === "tl" ? "cursor-nw-resize" : ""} ${handle.id === "br" ? "cursor-se-resize" : ""} ${handle.id === "tr" ? "cursor-ne-resize" : ""} ${handle.id === "bl" ? "cursor-sw-resize" : ""} bg-white border-2 border-blue-500 absolute`}
                      style={{
                        left: handle.left,
                        right: handle.right,
                        top: handle.top,
      ...
      {line.map((lin, idx) => {
        return (
          <div
            ...
            className="shape h-1 w-25 rounded translate-[-50%] bg-[#d9d9d9] absolute"
          ></div>
        );
      })}
